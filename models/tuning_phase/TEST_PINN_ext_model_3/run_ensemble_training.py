...
def run_ensemble_training():
    ...
    for _ in range(config.training_config['ensemble_size']):
        ...
        if config.training_config['random_parameters'] == False:
            ...
            alt_max = int(np.random.uniform(config.training_config['altitudes_max'][0], config.training_config['altitudes_max'][1]+1))
            l1_lambda = np.random.choice(config.training_config['l1_lambdas'])
            num_neurons_per_layer = np.random.uniform(config.training_config['nums_neurons_per_layer'][0], config.training_config['nums_neurons_per_layer'][1]+1)
        ...
        if config.training_config['validation']:
            ...
            train_loader, val_loader = bootstrap_sampling.split_wrt_orbits(input, target, orbit_nb, 
                                                                                    batch_size,
                                                                                    n_cpus,
                                                                                    )
